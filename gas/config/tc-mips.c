		     ...
		     unsigned int *channels_ptr)
{
  ...
  if (is_name_beginner (*e))
    ++e;
  while (is_part_of_name (*e) && *e != '.') // !TODO does this extra condition break anything
    ++e;
  ...
  if (mips_parse_register (&s, &regno1, &channels))
      ...
      else if (*s == '.') /* Check for N64 RSP extended vector index.  */
	{
	  ...
	  if (*s == 'e' || *s == 'h' || *s == 'q' || *s == 'b')
		...
		{
		  set_insn_error(0, _(". should be followed by e,h,q or b")); /// !TODO error message
		  return 0;
		}
			  ...
			  const struct mips_operand *operand, unsigned int uval)
{
  ...
  switch (qual)
  {
    ...
    case 'q':
      ...
      break;
    case 'b':
      set_insn_error (arg->argnum, _("vector computational instructions do not support byte indexing"));
      return false;
    default:
      return false;
  }
  ...
  if (e > emax)
{
  ...
  if ((arg->token->type != OT_CHAR || arg->token->u.ch != ',') && arg->token->type != OT_END)
    {
      set_insn_error (arg->argnum, _("vector register operand does not terminate appropriately"));
      return false;
    }
			  ...
			  const struct mips_operand *operand)
{
  ...
  if (arg->token->type != OT_REG || !match_regno (arg, OP_REG_RSP_VEC, arg->token->u.regno, &regno))
    return false;
  ...
  /* There may be OT_INTEGER_INDEX (qualified by either 'q' or 'h') as the next token, or nothing.  */
  if (arg->token->type == OT_INTEGER_INDEX)
    {
      emax = 7;
      ebase = 1 << 3;
      ...
      if (arg->token->type == OT_CHAR && arg->token->u.ch != ',')
	{
	  switch (arg->token->u.ch)
	    ...
	    case 'h':
	      ...
	      break;
	    default:
	      set_insn_error (arg->argnum, _("invalid vector element qualifier, should be 'q' or 'h' or none"));
	      return false;
	    }
	}
    }
...
static bool
can_swap_rsp_su_vu (struct mips_cl_insn *ip)
{
...
macro (struct mips_cl_insn *ip, char *str)
{
  ...
  switch (mask)
    ...
    ld_st_rsp:
	  macro_build (&offset_expr, s, fmt, op[0], op[1],
		       BFD_RELOC_RSP_7, op[3]);
